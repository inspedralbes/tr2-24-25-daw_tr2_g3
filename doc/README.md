...
* **Back-end (servidor)**: Utilitzem Laravel com a API per gestionar la comunicació amb una base de dades. Aquesta API
  ...
  creació, lectura, actualització i eliminació de dades (CRUD). Per un altre banda també utilitzem Node.js amb Socket.IO
  per a l'actualització de les dades a temps real. Aquesta arquitectura no gestiona dades sensibles dels clients.

* **Front-end (client)**: Utilitzem Vue 3 per gestionar la interfície d'usuari. Vue s'encarrega de la part interactiva i
...
* **Node.js**: Entorn d'execució de JavaScript del costat del servidor que permet crear aplicacions escalables i
  eficients. Basat en el motor V8 de Google, Node.js és conegut per la seva arquitectura asíncrona i basada en
  esdeveniments, ideal per a aplicacions amb necessitats d'entrada/sortida intensives.
...
1. **Instal·lació de Dependències**:
    * Accedeix al directori back/Syncblend-Laravel i executa composer install per instal·lar totes les dependències
...
3. **Configuració del Port**:
    * Assegura't que el port especificat al fitxer de configuració o al codi (`process.env.PORT` o un port per defecte,
      com ara 3000) estigui disponible.
...
2. **Comprimir en un zip la carpeta dist**

    * Anirem a `front/` i dintre s'haurà generat la carpeta `dist/` que haurem de comprimir.
    * Per comprimir la carpeta dist executarem la sgüent comanda:
   ```bash
    sudo zip -r dist.zip dist/
    ```
3. **Pujar els Fitxers al Servidor**:
    * Puja l'arxiu `dist.zip` al servidor dins de la carpeta `public_html` o la ubicació on es vol
      desplegar l'aplicació.
4. **Descomprimir dist.zip al directori public_html**

    * Per descomprimir executarem el següent:
   ```bash
    unzip dist.zip
    ``` 
   
5. **Moure el contingut**
   * Mourem tot el contingut de `dist/` al directori `public_html`.

6. **Assegurar-se que el Servei està en Funcionament**:
    * Accedeix a l'URL corresponent (`http://el_teu_domini.com`) per verificar que l'aplicació Vue 3 està funcionant
      correctament.
...
* Per mantenir obert el port de Node.js en producció, segueix aquests passos:
    *En primer cop farà falta obrir una nova terminal amb __screen__, per obrir-lo executarem:
  ``` bash 
    screen  
  ```

    *  Per a signar el port caldrà anar a l'arxiu __index.js__ i la constant __port__ canviar-lo per un altre, del 20000 al 30000.
    * Per exemple:
  ...
  ```
    ...
    * Per mantenir el servei en marxa caldrà executar la següent comanda:

  ``` bash 
    node index.js  
  ```

    * Perquè el servidor node continuï en marxa caldrà fer `CTRL + A + D` perquè no es tanqui la terminal i es mantingui en segon pla.
    * Això desacoblarà la terminal perquè s'executi independentment.
...
4. **Modificar el Fitxer index.php**:
    ...
    * Copia la ruta completa fins a private/back/Syncblend-Laravel/ i substitueix totes les referències que comencin
      amb .. amb la nova ruta copiada.
